...
export class TrackComponent implements OnInit {
  ...
  trackId = (_index: number, item: ITrack): string => this.trackService.getTrackIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.tracks = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ITrack[] | null): ITrack[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.trackService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
